...
| Suggested NEW path | `C:\activerse\led-hexagon` (or `...\led-hexagon-git` if OLD already uses that folder) |
...
> Examples: `C:\activerse\led-hexagon`, `D:\led-hexagon`, a Desktop extract.  
...
(e.g. `C:\activerse\led-hexagon-git`) so you do not overwrite OLD before audit.
...
```powershell
$NEW = "C:\activerse\led-hexagon"          # adjust to actual NEW path
...
UI: `http://localhost:5177`. Stop: `STOP_GAME.bat`.
