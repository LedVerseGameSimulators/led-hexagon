# AGENT_SETUP_AND_RUN.md — LED Hexagon (Windows game machine)
...
|------|-------|
| Repo | `led-hexagon` |
| Clone URL | `https://github.com/LedVerseGameSimulators/led-hexagon.git` |
| Suggested path | `C:\activerse\led-hexagon` |
| API / Bridge / UI | `8004` / `8767` / `5177` |
...
> What is the full path to the **existing / old** LED Hexagon install on this PC  
> (the codebase that was running here before today’s git clone)?  
> Examples: `C:\activerse\led-hexagon`, `D:\led-hexagon`, a Desktop extract folder.  
...
Prefer a **new** folder if OLD already occupies `C:\activerse\led-hexagon`, e.g.
`C:\activerse\led-hexagon-git`, then swap after audit. Or rename OLD aside first.

```bat
git lfs install
mkdir C:\activerse 2>nul
cd C:\activerse
git clone https://github.com/LedVerseGameSimulators/led-hexagon.git
cd led-hexagon
git checkout main
git pull
git lfs pull
...
```powershell
$NEW = "C:\activerse\led-hexagon"          # the git clone (after git lfs pull)
$OLD = "C:\path\to\OLD_ROOT"             # set from Phase 1
$Report = "C:\activerse\hexagon-reconcile-report.txt"
...
UI: `http://localhost:5177` — leave minimized windows open.  
Stop: `STOP_GAME.bat`.
