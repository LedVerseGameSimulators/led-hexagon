    ...
    def init_hw_led(self, setting):
        if Setting.USE_SERIAL_HD:
            ...
            for serial in serial_list_error:
                logger.warning(f"Serial port failed (wall light): {serial}. Continuing in headless mode.")

            self.led_col_wall_screen = LedControl()
            serial_list_error = self.led_col_wall_screen.init_com(setting.list_screen_com_info)
            for serial in serial_list_error:
                logger.warning(f"Serial port failed (wall screen): {serial}. Continuing in headless mode.")
            ...
            for serial in serial_list_error:
                logger.warning(f"Serial port failed (main): {serial}. Continuing in headless mode.")
...
# okay decompiling /Users/apple/parallel-work/decompile_workspace/ledhexagon.exe_extracted/PYZ-00.pyz_extracted/game_play/game_hw.pyc
