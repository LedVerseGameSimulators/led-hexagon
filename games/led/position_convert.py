...
# Embedded file name: position_convert.py
"""
问题：
...
def wall_light_logic2real(logic_arr, start=1, direct='right'):
    real_arr = []
    size = len(logic_arr)
    if direct == "right":
        for x in range(size):
...
def wall_light_real2logic(real_arr, start=1, direct='right'):
    logic_arr = []
    size = len(real_arr)
    if direct == "right":
        for y in range(size):
...
# okay decompiling /Users/apple/parallel-work/decompile_workspace/ledhexagon.exe_extracted/PYZ-00.pyz_extracted/led/position_convert.pyc
