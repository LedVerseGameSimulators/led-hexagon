...
# Embedded file name: gui_game.py

import importlib, os, datetime, sys, threading, time, traceback
from tkinter import messagebox
from loguru import logger
import game_play.Play as Play
from gui.gui_editor_game import EditorGame
from gui.gui_editor_game2 import EditorGame2
...
class GuiGame:
    ...
    def play_music(self, ms_path, waitting=False, root=None):
        try:
    ...
    def music(self, stage, waitting=False, root=None, loops=-1):
        logger.info("music")
        if stage == "introduce":
            try:
    ...
    def game_running(self, game_path, game_time, player_min_nums, barcode, life_value):
        self.dir_name = os.path.splitext(os.path.basename(game_path))[0]
        module_name = game_path.split(".")[-1]
        if module_name == "led":
            logger.info("in gui EditorGame")
            self.obj_module = EditorGame((self.parent), self, life_value, [], (self.game_time), player_min_nums, last_root=(self.last_root),
              game_unzip_dir=game_path)
            logger.info("out of gui EditorGame")
        else:
            if module_name == "ledb":
                logger.info("in gui EditorGame2")
                self.obj_module = EditorGame2((self.parent), self, life_value, [], (self.game_time), player_min_nums, last_root=(self.last_root),
                  game_unzip_dir=game_path)
        ...
        if self.last_root:
            if self.last_root != self.parent.root:
                self.last_root.destroy()
        self.last_module = self.obj_module

    def game_scode_rule_old(self, scode, b_divide_person, b_divide_time, person_nums, time):
        scode = scode
        if b_divide_person:
            if person_nums != 0:
                scode /= person_nums
    ...
    def db_operation_after_game(self, game_nums):
        ...
        self.str_time = time_now.__format__("%Y-%m-%d %H:%M:%S")
    ...
    def game_result(self):
        # Show results whenever a game actually ran — not just when time ran out.
        # Pattern-completion ends the game with time_left still positive, so we
        # check that the game timer was started instead.
        if self.game_record_rt.game_time_start > 0 or self.game_record_rt.game_time_left <= 0:
            logger.info("game ended — saving result (time_left={}, time_start={})",
                        self.game_record_rt.game_time_left, self.game_record_rt.game_time_start)
            ...
            if self.barcode and self.game_record_rt.game_state == Setting.GAME_NORMAL:
                ...
                try:
                    ...
                    GameResult((self.parent), (self.db), result, (self.player_group_id), (self.dir_name), game_time, (len(self.list_phone_num)),
                      cur_scode=scode, barcode=(self.barcode), parent_game_running=self)
                except:
                    logger.error("{}", traceback.format_exc())
                ...
                try:
                    ...
                    player_name = self.player_list[0] if self.player_list else "Guest"
                    tourist = Tourist(player_name)
                    tourist.scode = scode
                    tourist_record.add(tourist)

                    # ── rich leaderboard record ────────────────────────────
                    try:
                        rt = self.game_record_rt
                        time_played_sec = round(rt.get_game_time_pass() * 60, 1)
                        tourist_record.add_game_result(
                            game_name    = self.dir_name or "Unknown",
                            level        = str(self.setting.game_leval.get()),
                            player_count = len(self.player_list),
                            players      = ", ".join(self.player_list) if self.player_list else "Guest",
                            duration_min = game_time,
                            time_played_sec = time_played_sec,
                            score        = scode,
                            score_p1     = rt.game_scode_left if rt.game_scode_left else None,
                            score_p2     = rt.game_scode_right if rt.game_scode_right else None,
                            lives_start  = self.life_value,
                            lives_left   = self.life_value - rt.game_blood if rt.game_blood else self.life_value,
                        )
                        logger.info("leaderboard record saved: game={} score={}", self.dir_name, scode)
                    except Exception:
                        logger.error("leaderboard write failed: {}", traceback.format_exc())
                    # ──────────────────────────────────────────────────────

                    result = tourist_record.search(limit=100000)
                    tourist_record.close()
                    tourist_record.dispaly((self.parent), result, (self.dir_name), game_time, (len(self.player_list)), scode, parent_game_running=self)
                except:
                    logger.error("{}", traceback.format_exc())

    def start_game(self, setting, game_time, player_list, player_min_nums, barcode, life_value):
        """
        Called from GuiGame.__init__() after hardware is initialised.
        Iterates through self.list_game, runs each game, then shows the result.
        player_list contains phone numbers (barcode mode) or tourist names.
        """
        logger.info("GuiGame.start_game games={}", len(self.list_game))
        self.player_list = player_list
        # list_phone_num is used for database operations (barcode mode)
        self.list_phone_num = list(player_list) if barcode else []
        self.barcode = barcode
        self.game_time = game_time
        self.life_value = life_value
        self.circle_game = True
        list_game_tmp = list(self.list_game)

        for game_path in list_game_tmp:
            if not self.circle_game:
                break
            try:
                self.cur_game_name = os.path.basename(game_path)
                logger.info("start_game: running '{}'", self.cur_game_name)
                # game_music_init needs a parsed Game object, NOT the raw path.
                # It is already called correctly inside EditorGame2.play_running()
                # and EditorGame.play_running() after the game file is loaded.
                self.game_running(game_path, game_time, player_min_nums, barcode, life_value)
                self.game_nums += 1
            except Exception:
                logger.error("start_game loop: {}", traceback.format_exc())

        self.game_result()
        logger.info("GuiGame.start_game complete")

    def get_hw_led_state_inc_wal(self, led_table_obj):
        if Setting.USE_SERIAL_HD:
            ret = led_control.get_led_state(None, led_table_obj.get_state_table(), None, None)
            if ret == 0:
                self.reopen_com()
    ...
    def draw_hw_led_color_inc_wal(self, led_table_obj):
        if Setting.USE_SERIAL_HD:
            ...
            if self.setting.screen.get():
                self.led_col_wall_screen.update_wall_screen(led_table_obj.get_wall_screen_arr(), self.setting.wall_light_layout_real)
            logger.debug("end")
    ...
    def __init__(self, parent, life_value, list_game, barcode, game_time, player_list, last_root=None):
        ...
        # attributes used across game lifecycle methods
        self.barcode = barcode
        self.dir_name = ""   # game display name, set in game_running()
        self.str_time = ""   # game start timestamp, set in db_operation_after_game()
        ...
        if not self.init_hw_led():
            ...
            return
        if self.game_record_rt.game_state == Setting.GAME_NORMAL:
            if barcode:
                if self.game_record_rt.game_time_start == 0:
    ...
    def play_running(self, partial_update, game, dict_group, game_folder_path, led_table_obj=None, root=None):
        setting = self.setting
        self.play = Play(led_table_obj, setting, partial_update, game_level=(self.game_level))
...
# file /Users/apple/parallel-work/decompile_workspace/ledhexagon.exe_extracted/PYZ-00.pyz_extracted/gui/gui_game.pyc
