...
class EditorGame:
    ...
    def smaller(self, num1, num2):
        ACCURACY = 1e-06
        smer = num2 - num1 > ACCURACY
        return smer

    def group_in_time(self, start_time, end_time):
        ACCURACY = 1e-06
        tmp_total = self.total_pass
        bigger = tmp_total - start_time >= ACCURACY or tmp_total - start_time >= -ACCURACY
        smaller = end_time - tmp_total > ACCURACY
    ...
    def frame_auto_jump(self):
        next_goal_group_start = EditorGame.MAX_TIME
        for member in self.list_start_time_goal_group.copy():
            ...
            if start_time < self.total_pass:
                ...
                self.list_group_breath[0].set_color(color)
            else:
        ...
        for key, value in self.dict_group.items():
            group = value
            if group.type == Setting.FLOOR_LIGHT and group.color == self.ui.led_table.goal_color and len(group.start_member) > 0:
        ...
        if self.total_pass == EditorGame.MAX_TIME:
            ret = True
        return ret

    def clear_led_table(self, color=[
 Color.BLACK] * 3):
        led_table = self.ui.led_table.led_table
    ...
    def running_by_time(self, dict_group):
        ...
        while self.running_state and self.circle_game:
            ...
            for key, value in dict_group.items():
                ...
                if group.speed != 0:
                    ...
                    # First-frame precision: clamp to actual elapsed in group.
                ...
                if self.group_in_time(start_time, end_time):
                    # Always accumulate fractional movement (was inside dead branch).
            ...
            _frame_elapsed = time.time() - self.current_time
    ...
    def running_by_blue(self, dict_group):
        ...
        period_check_blue_exist = 0
        self.check_count_time = 1
        ...
        while self.running_state and self.circle_game:
            ...
            next_blue_group_start = MAX_TIME
            ret = self.frame_auto_jump()
            if not ret:
            ...
            if ret:
                    ...
                    if self.group_in_time(group.start_time_sec, group.end_time_sec):
                        if group.type == Setting.FLOOR_LIGHT:
                            if group.speed != 0:
                                ...
                                if 0 < tmp_time < time_pass:
                                    change = tmp_time * last_speed
                            # Always accumulate (was inside dead elif branch).
                            group.move_distance += change
                            if self.smaller(group.move_distance, 1):
                                continue
                            else:
                ...
                if _sleep > 0.001:
                    time.sleep(_sleep)
                # Keep Tkinter UI alive while the blocking loop owns the main thread.
                try:
                    self.ui.root.update_idletasks()
                except Exception:
                    pass
    ...
    def running(self, game, dict_group, game_folder_path):
        ...
        if game.play_order:
            self.running_by_time(dict_group)
        else:
            self.running_by_blue(dict_group)
    ...
    def play_running(self, game_unzip_dir, setting, partial_update_ui):
        ...
        try:
            for game_folder in os.listdir(game_unzip_dir):
                ...
                if self.circle_game:
                    ...
                    self.cover_color = [color_tmp, [Color.RED] * 3]
                    ...
                    # Initialise the game timer before the loop starts (mirrors the
                    # EditorGame2 fix — game_time_left starts at 0 in GameRecordRT).
                    self.game_record_rt.game_time_left = self.game_time * 60
                    self.running(game, dict_group, game_folder_path)
                    if self.circle_game:
                        ...
                        if game_end is not None and dict_group_end is not None:
                            ...
                            self.callback = tmp_partial_update
                            self.running(game_end, dict_group_end, game_end_dir, led_table_obj, self.ui.root)
        ...
        logger.info("EditorGame1 end play_running")
    ...
    def update_draw_led_table_end_of_play(self):
        try:
            ...
            self.game_record_rt.game_time_left = int(self.game_time * 60 - (time.time() - self.game_start_time))
    ...
    def draw_code_group(self, time_pass):
        ...
        # Decompiler inverted this condition. Original: if breath groups exist, animate.
    ...
    def color_cover_over_times_prompt(self, dict_group, time_cur):
        ...
        for group in list_group:
            if group.type == Setting.FLOOR_LIGHT and group.speed == 0:
                if group.start_time_sec + time_threshold < time_cur < group.end_time_sec:
                    if group.color in [o_led_table.goal_color, Color.DEDUCT_COLOR]:
            ...
            if group.start_time_sec < cur_time < group.end_time_sec:
                if group.type == Setting.FLOOR_LIGHT:
                    if group.color == [Color.RED] * 3:
                        for coors in group.start_member.copy():
                            i = coors[0]
                            j = coors[1]
                            if table_state[i][j] and cur_time - self.life_cal.table_count_time[i][j] > self.life_cal.TIME:
                                ...
                                print(f"cur_time{cur_time} record time {self.life_cal.table_count_time[i][j]}")
                ...
                for coors in group.start_member.copy():
                    ...
                    if table_state[i][j]:
                        ...
                        blink_group = LedGroup([(i, j)], color=(group.color))
                        self.list_group_blink.append(blink_group)

            elif group.color == self.goal_color:
                for coors in group.start_member.copy():
                    i = coors[0]
                    j = coors[1]
                    if table_state[i][j]:
                ...
                for coors in group.start_member.copy():
                    ...
                    j = coors[1]
                    if table_state[i][j]:
                        red_table[i][j] or audio.Audio().play(audio_name)
                        if safe_table[i][j]:
                            group_tmp = LedGroup([(i, j)], color=(group.color), life_period=hidn_tread_show_time,
                              is_living=True)
                            hidden_tread_show.append(group_tmp)
    ...
    def callback_in_every_frame(self, parent, dict_group, time_pass, total_pass):
        try:
            led_table = self.ui.led_table
            self.goal_color = led_table.goal_color
    ...
    def update_hw_led_new(self, game_record):
        try:
            if Setting.USE_SERIAL_HD:
                ...
                # Push the current led_table frame to the browser simulator,
                # then refresh the Tkinter score overlay.
    ...
    def __init__(self, parent, parent_last, life_value, list_game, game_time, player_min_num, last_root=None, game_unzip_dir=''):
        ...
        self.led_blink_group = LedBlinkGroup(self.ui.led_table.led_table, [Color.RED_BLINK] * 3, [
         Color.BLACK] * 3)
        ...
        group_tmp = LedGroup([pst_goal], color=([self.ui.color_hex_str2int_arr(self.setting.hidden_prompt_breath_color.get())] * 3))
        ...
        # Always start a fresh score — never carry over the previous game's value.
...
# okay decompiling /Users/apple/parallel-work/decompile_workspace/ledhexagon.exe_extracted/PYZ-00.pyz_extracted/gui/gui_editor_game.pyc
